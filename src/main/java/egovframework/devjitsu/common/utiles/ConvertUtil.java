...
public class ConvertUtil {
        ...
        try {
            ...
            if(servletRequest.getServerName().contains("localhost") || servletRequest.getServerName().contains("127.0.0.1")){
                ...
                if(prevFile != ""){
                    ...
                    if(params.containsKey("receiveTmplate")) {
                        fileUUID = prevFile;
                    }
                }

                if(params.containsKey("receiveTmplate")){
                    System.out.println("                             if(params.containsKey(\"receiveTmplate\")){                                   ");
                    if(!StringUtils.isEmpty(params.get("docId"))){
                        docFilepath = "approveDocFile/" + params.get("menuCd").toString()+"/" + params.get("docId");
                    }else{
                        docFilepath = "approveDocFile/" + params.get("menuCd").toString()+"/" + params.get("DOC_ID");
                    }
                    if(servletRequest.getServerName().contains("localhost") || servletRequest.getServerName().contains("127.0.0.1")){
                    }else if (servletRequest.getServerName().contains("121.186.165.80")){
                        docFilepath = "/home" + base_dir + docFilepath + "/";
                    }else{
                        docFilepath = "/nas1" + base_dir + docFilepath + "/";
                    }
                }
    ...
    public String approveDocFilePath(HttpServletRequest servletRequest, Map<String, Object> params, String base_dir){
        ...
        }else if(!StringUtils.isEmpty(params.get("receiveTmplate"))){
            path = "approveDocFile/" + params.get("menuCd").toString()+"/" + params.get("receiveTmplate");
        }else{

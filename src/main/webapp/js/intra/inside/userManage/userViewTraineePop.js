var userViewTraineePop = {
    ...
    dataSet : function() {
        ...
        $("#deptComp").data("kendoDropDownList").bind("change", userViewTraineePop.fn_chngDeptComp)
        $("#deptComp").data("kendoDropDownList").select(0);
        $("#deptComp").data("kendoDropDownList").trigger("change");

        userViewTraineePop.global.dropDownDataSource = customKendo.fn_customAjax("/system/commonCodeManagement/getCmCodeList", {cmGroupCodeId : "3"});
        customKendo.fn_dropDownList("duty", userViewTraineePop.global.dropDownDataSource, "CM_CODE_NM", "CM_CODE", "4");
    },
    ...
    setUserDel : function(e){
        if(confirm("삭제된 직원은 복구가 불가능합니다.\n직원을 삭제처리하시겠습니까?")){
            userViewTraineePop.global.saveAjaxData = {
                empSeq : e,
                regEmpSeq : $("#regEmpSeq").val()
            }

            var result = customKendo.fn_customAjax("/userManage/setUserDel.do", userViewTraineePop.global.saveAjaxData);
            if(result.flag){
                alert("처리되었습니다.");

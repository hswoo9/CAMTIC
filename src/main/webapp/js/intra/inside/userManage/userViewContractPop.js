var userViewContractPop = {
    ...
    setUserDel : function(e){
        if(confirm("삭제된 직원은 복구가 불가능합니다.\n직원을 삭제처리하시겠습니까?")){
            userViewContractPop.global.saveAjaxData = {
                empSeq : e,
                regEmpSeq : $("#regEmpSeq").val()
            }

            var result = customKendo.fn_customAjax("/userManage/setUserDel.do", userViewContractPop.global.saveAjaxData);
            if(result.flag){
                alert("처리되었습니다.");

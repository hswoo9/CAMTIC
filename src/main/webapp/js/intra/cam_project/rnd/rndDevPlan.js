var rndDP = {
    ...
    fn_setData: function (key){
        ...
        for(var i = 0 ; i < list.length ; i++){
            ...
            html += '   <td style="text-align: center"><span style="position: relative; top:5px">'+idx+'</span></td>' +
                ...
                '           <button type="button" id="" class="k-grid-button k-button k-button-md k-button-solid k-button-solid-base" onclick="rndDP.fn_popCamCrmList('+idx+')">\n' +
                '              조회' +
                '           </button>' +
                '       </td>\n' +
                '       <td><input type="text" id="invEtc'+idx+'" class="invEtc" /></td>\n' +
                '       <td style="text-align: center;">' +
                '           <button type="button" id="modBtn" onclick="rndDP.fn_modInv('+list[i].INV_SN+','+idx+')" class="k-button k-button-solid-primary">수정</button>' +
    ...
    fn_buttonSet : function(devMap){
        ...
        if(devMap != null){
            ...
            if(status == "0"){
                ...
                buttonHtml += "<button type=\"button\" id=\"devDelBtn\" style=\"float: right; margin-bottom: 5px;\" class=\"k-button k-button-solid-error\" onclick=\"devInfo.fn_delete()\">삭제</button>";
                buttonHtml += "<button type=\"button\" id=\"devSaveBtn\" style=\"float: right; margin-right: 5px;\" class=\"k-button k-button-solid-info\" onclick=\"rndDP.fn_save()\">저장</button>";
                buttonHtml += "<button type=\"button\" id=\"devReBtn\" style=\"float: right; margin-right: 5px;\" class=\"k-button k-button-solid-error\" onclick=\"tempOrReDraftingPop('"+devMap.DOC_ID+"', '"+devMap.DOC_MENU_CD+"', '"+devMap.APPRO_KEY+"', 2, 'reDrafting');\">재상신</button>";
            }else if(status == "100"){
                ...
                if (status == "0") {
                    if(commonProject.global.devTeamCk != "Y") {
                        if(rndDP.global.invCk == "Y") {
                            ...
                            buttonHtml += "<button type=\"button\" id=\"devSaveBtn\" style=\"float: right; margin-right: 5px\" class=\"k-button k-button-solid-info\" onclick=\"rndDP.fn_save()\">저장</button>";
                            buttonHtml += "<button type=\"button\" id=\"addVerBtn2\" style=\"float: right; margin-bottom: 5px; margin-right: 5px;\" class=\"k-button k-button-solid-base\" onclick=\"rndDP.fn_addVersion()\">수행계획서 추가</button>";
                            buttonHtml += "<button type=\"button\" id=\"devTeamAppBtn\" style=\"float: right; margin-right: 5px\" class=\"k-button k-button-solid-info\" onclick=\"rndDP.fn_teamApp('Y')\">공정 마감</button>";
                        }else{
                            buttonHtml = "<button type=\"button\" id=\"devDelBtn\" style=\"float: right; margin-bottom: 5px;\" class=\"k-button k-button-solid-error\" onclick=\"devInfo.fn_delete()\">삭제</button>";
                            buttonHtml += "<button type=\"button\" id=\"devSaveBtn\" style=\"float: right; margin-right: 5px\" class=\"k-button k-button-solid-info\" onclick=\"rndDP.fn_save()\">저장</button>";
                        }
                    }else {
                        buttonHtml = "<button type=\"button\" id=\"devTeamAppBtn\" style=\"float: right; margin-bottom: 10px\" class=\"k-button k-button-solid-error\" onclick=\"rndDP.fn_teamApp('N')\">마감취소</button>";
        ...
        if(devMap != null && devMap.DOC_ID != null){
            reDraftOnlyOne(devMap.DOC_ID, $("#devRegEmpSeq").val(), "devReBtn");
        }
    },
    ...
    selCrmInfo :  function(e){
        $("#estOfc" + rndDP.global.crmIdx).val(e.CRM_NM);
        rndDP.global.crmIdx = "";
    },

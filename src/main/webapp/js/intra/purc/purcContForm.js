...
function purcContRepFormPop(purcReqId, purcReqType, formId){
	...
	var strResize = 0 ;
	pop = window.open(url, '구매계약보고', "width=1200, height=" + height + ", top="+ windowY +", left="+ windowX +", resizable="+ strResize);
...
purcReq.setPurcContInfo = function(purcContInfo){
	$('#contTitle').val(purcContInfo.CONT_TITLE);
	if(purcContInfo.PAY_CON_CODE_ID){
		if($("#payCon").data("kendoComboBox")){
			$("#payCon").data("kendoComboBox").value(purcContInfo.PAY_CON_CODE_ID);
		}else{
			$("#payCon").val(purcContInfo.PAY_CON).attr('code', purcContInfo.PAY_CON_CODE_ID);
		}
	}
	if(purcContInfo.PAY_TYPE_CODE_ID){
		if($("#payType").data("kendoComboBox")){
			$("#payType").data("kendoComboBox").value(purcContInfo.PAY_TYPE_CODE_ID);
		}else{
			$("#payType").val(purcContInfo.PAY_TYPE).attr('code', purcContInfo.PAY_TYPE_CODE_ID);
		}
	}
	$('#payCnt').val(purcContInfo.PAY_CNT);
	$('#contAm').val("30,000");
	if(purcContInfo.CONT_AM){
		$('#contAm').val(purcContInfo.CONT_AM.toString().toMoney());
	}
	if(purcContInfo.CONT_TYPE_CODE_ID){
		if($("#contType").data("kendoComboBox")){
			$("#contType").data("kendoComboBox").value(purcContInfo.CONT_TYPE_CODE_ID);
		}else{
			$("#contType").val(purcContInfo.CONT_TYPE).attr('code', purcContInfo.CONT_TYPE_CODE_ID);
		}
	}
	if($("#contEndDate").data("kendoDatePicker")){
		$("#contEndDate").data("kendoDatePicker").value(ncCom_Date(purcContInfo.CONT_END_DATE, '-'));
	}else{
		$("#contEndDate").val(ncCom_Date(purcContInfo.CONT_END_DATE, '-'));
	}
	$('#contTerm').val(purcContInfo.CONT_TERM);
	if($("#contDate").data("kendoDatePicker")){
		$("#contDate").data("kendoDatePicker").value(ncCom_Date(purcContInfo.CONT_DATE, '-'));
	}else{
		$("#contDate").val(ncCom_Date(purcContInfo.CONT_DATE, '-'));
	}
	$('#txt_TR_NM_cont').val(purcContInfo.TR_NM).attr('code', purcContInfo.TR_CD);
	$('#txt_REG_NB_cont').val(purcContInfo.REG_NB);
	$('#txt_CEO_NM_cont').val(purcContInfo.CEO_NM);
	...
	if(purcContInfo.CONT_STEP == '003'){
		$('#btnConc').show();
	}else{
		$('#btnConc').hide();
	}
	if(processId == 'tpfContConc'){
		if(purcContInfo.CONT_DATE){
			$("#contDate2").data("kendoDatePicker").value(ncCom_Date(purcContInfo.CONT_DATE, '-'));
			$("#contStartDate2").data("kendoDatePicker").value(ncCom_Date(purcContInfo.CONT_DATE, '-'));
		}
		if(purcContInfo.CONT_END_DATE){
			$("#contEndDate2").data("kendoDatePicker").value(ncCom_Date(purcContInfo.CONT_END_DATE, '-'));
		}
		if(purcContInfo.CONT_DATE2){
			$("#contDate2").data("kendoDatePicker").value(ncCom_Date(purcContInfo.CONT_DATE2, '-'));
		}
		if(purcContInfo.CONT_TERM && $("#contDate2").val()){
			var date = new Date(purcContInfo.CONT_DATE);
			date.setDate(date.getDate() + parseInt(purcContInfo.CONT_TERM));
			var contEndDate = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate();
			$("#contEndDate2").data("kendoDatePicker").value(contEndDate);
		}
		if(purcContInfo.CONT_START_DATE2){
			$("#contStartDate2").data("kendoDatePicker").value(ncCom_Date(purcContInfo.CONT_START_DATE2, '-'));
		}
		if(purcContInfo.CONT_END_DATE2){
			$("#contEndDate2").data("kendoDatePicker").value(ncCom_Date(purcContInfo.CONT_END_DATE2, '-'));
		}
	}else if(processId == "view"){
		$("#contDate2").val(ncCom_Date(purcContInfo.CONT_DATE2, '-'));
		$("#contStartDate2").val(ncCom_Date(purcContInfo.CONT_START_DATE2, '-'));
		$("#contEndDate2").val(ncCom_Date(purcContInfo.CONT_END_DATE2, '-'));
	}

	/** 제거 필요 */
	$("#expectedAm").val("30,000");
	if(purcContInfo.EXPECTED_AM){
		$("#expectedAm").val(purcContInfo.EXPECTED_AM.toString().toMoney());
	}
	
	/*소재지 추가*/
	if(purcContInfo.TR_PLACE){
		if($("#trPlace").data("kendoComboBox")){
			$("#trPlace").data("kendoComboBox").value(purcContInfo.TR_PLACE);
		}else{
			$("#trPlace").val(purcContInfo.TR_PLACE);
		}
	}
	/*소재지 추가*/
};

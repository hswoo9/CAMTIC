...
function fnMgtCdSet(sel, dblClickparamMap){
	...
	if(PJT_CD != $("#temp_PjtCd").val()){
		...
		if(tr.attr('id') && tr.attr('id') != 0){
			...
			/*ajax 호출할 파라미터*/
			var opt = {
					url : getContextPath() + "/Ac/G20/Ex/delPurcReqH.do",
					stateFn : modal_bg,
					async : false,
					data  : data,
...
abdocu.BudgetInfo.init = function(noInitSet, focus){
    ...
    var opt = {
            url : getContextPath() + "/Ac/G20/Ex/getPurcReqB.do",            
...
abdocu.BudgetInfo.deleteRow = function(id){
    ...
    var opt = {
            url : getContextPath() + "/Ac/G20/Ex/delPurcReqB.do",
            stateFn : abdocu.state,
            async : false,
            data : data,
...
//    obj.consDocSeq   = consDocSeq;
	...
	/*ajax 호출할 파라미터*/
    var opt = {
            url : getContextPath() + "/Ac/G20/Ex/getBudgetInfo.do",
            stateFn:abdocu.state,
            async:false,
            data : obj,
            type:"post",
...
abdocu.TradeInfo.select = function(rowSelectID, eventid){
    ...
    var opt = {
            url : getContextPath() + "/Ac/G20/Ex/getPurcReqT.do",
            stateFn:abdocu.state,
            async:false,
            data : data,
            successFn : function(data){
...
abdocu.TradeInfo.deleteRow = function(id){
    ...
    var opt = {
            url : getContextPath() + "/Ac/G20/Ex/delPurcReqT.do",
            stateFn:abdocu.state,
            async:false,
            data : data,
            successFn : function(){
...
abdocu.TradeInfo.getETCDUMMY1 = function(parentEle){
    ...
    var opt = {
    		url : getContextPath() + "/Ac/G20/Ex/getETCDUMMY1.do",
    		stateFn:abdocu.state,
    		async:false,
    		data : data,
    		successFn : function(result){
...
abdocu.TradeInfo.getETCDUMMY1Info = function(parentEle){
    ...
    var opt = {
    		url : getContextPath() + "/Ac/G20/Ex/getErpETCDUMMY1_Info.do",
    		stateFn:abdocu.state,
    		async:false,
    		data : data,
    		successFn : function(result){
...
function fnGetConferBalance(id){
	...
	var opt = {
			url : getContextPath() + "/Ac/G20/Ex/getConferBalance.do",
			stateFn : abdocu.state,
			async : false,
...
function fnPurcReqApproval(){
	if(confirm('구매의뢰를 접수합니다.')){
	    ...
	    var opt = {
	    		url     : getContextPath() + "/Ac/G20/Ex/updatePurcReqState.do",
	            async   : false,
	            data    : params,
	            successFn : function(data){
					console.log("여기인가?");
	            	...
	            	if($('#contTypeCodeId').val() != '001'){
	            		...
	            		var opt = {
	            				url     : getContextPath() + "/Ac/G20/Ex/makeContInfo.do",
	            				async   : false,
	            				data    : obj,
	            				successFn : function(data){
...
function purcContRepFormPop(purcReqId, purcReqType, formId, purcContId){
	var url = getContextPath() + '/Ac/G20/Ex/purcContRepForm.do?focus=txt_BUDGET_LIST&form_tp=tpfContRep&purcReqId=' + purcReqId + '&purcReqType=' + purcReqType + '&form_id=' + formId + '&purcContId=' + purcContId;
	var pop = "" ;
	var width = "1000";
	var height = "950";
...
function fnTpfCommInit(){
	...
	var commCode = commCodeList.filter(function(obj){return obj.code == purcReqType;})[0];
	$('#purcReqType').val(commCode.code_kr);
	$('#purcReqType').attr('CODE',commCode.code);
}
...
purcReq.setPurcReq = function(purcReqInfo){
	...
	$('#purcReqNo').val(purcReqInfo.PURC_REQ_NO2);
	$('#purcReqTitle').val(purcReqInfo.PURC_REQ_TITLE);
	if(purcReqInfo.PURC_REQ_DATE != null){
	...
	$('#reqState').val(purcReqInfo.REQ_STATE);
	if(purcReqInfo.REQ_STATE == '003'){
		$('.psh_btnbox .psh_right').show();
	}else{
		$('.psh_btnbox .psh_right').hide();
	}
	$('#contType').val(purcReqInfo.CONT_TYPE);
	$('#contTypeCodeId').val(purcReqInfo.CONT_TYPE_CODE_ID);
	$('#contDate').val(purcReqInfo.CONT_DATE);
	$('#basicAm').val(purcReqInfo.BASIC_AM);
	if(purcReqInfo.CONT_AM != null){
...
function fnTpfAttachFileDownload(obj){
	...
	downWin.location.href = getContextPath() + '/common/fileDown?attach_file_id='+attach_file_id;
} 

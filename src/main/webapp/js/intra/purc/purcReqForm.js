...
abdocu.state = function(state){
    if(state){
        ...
        var img = $("<img>").attr("id", "g20-ajax-img").attr("src", getContextPath() + "/images/ico/loading.gif")
...
function fnAbdocuInit(){
    $.ajax({
        ...
        dataType: "json",
        async : false,
        url: "/purc/AcExDocInitSubData.do",
        data : {
            cmCode : purcFormCode
        },
        success: function (res) {
            ...
            tblParam.SubBizYN = res.SubBizYN;
            tblParam.ItemsUseYn = res.ItemsUseYn;
            tblParam.ACardUseYn = res.ACardUseYn;
            tblParam.empSeq = $("#empSeq").val();

            if(res.resultValue =="notTempKey") {
                alert("등록되지 않은 양식입니다. 관리자에게 문의 바랍니다");
                self.close();
            }else{
                $.ajax({
                    type: "POST"
                    , dataType: "json"
                    , url: "http://121.186.165.80:8080/Ac/G20/Ex/AcExDocInit"
                    , data: tblParam
                    , async: false
                    , global: false
                    , success: function (res) {
...
abdocu.checkBClose = function(fn, tr){
    ...
    var opt = {
        url : "http://121.186.165.80:8080/Ac/G20/Ex/chkErpBgtClose.do",
...
abdocu.ProjectInfo.RowSave_Process = function(tr){
    ...
    /** 구매계약 그룹 키 */
    saveObj.purcGroupId = purcGroupId;
    /** 구매계약 이전 구매계약정보키 */
    saveObj.prevPurcId = prevPurcId;
    ...
    saveObj.purcBsTitle = $('#purcBsTitle').val();
    saveObj.purcBsContent = CKEDITOR.instances.purcBsContent.getData();
...
//	saveObj.abdocu_no       = abdocuInfo.abdocu_no;
    ...
    saveObj.purcBusnId      = $(".purcBusnId", tr).val();
    ...
    saveObj.targetFunction = "rowSaveProcess";

    var resultData = {};
    /*ajax 호출할 파라미터*/
    var opt = {
        url : getContextPath() + "/purc/setPurcInfo.do",						// 구매의뢰서 저장
        stateFn : abdocu.state,
        async: true,
        data : saveObj,
        successFn : function(data){
            if(data){
                ...
                $(".purcBusnId", tr).val(data.purcBusnId);

                purcReqId = data.purcReqId ? data.purcReqId : "0";
...
function excelTemplateDown(){
    ...
    downWin.location.href = _g_contextPath_ + '/purc/getEexcelTemplateDown.do?purcReqType='+purcReqType+'&purcTrType='+purcTrType;
}
...
function fnTpfCommInit(){
    ...
    var commCdoe = Array.isArray(commCodeList) ? commCodeList.filter(function(obj){return obj.code == purcReqType;})[0] : [];
...
function fnPurcReqInfo(){
    if(purcReqId == 0){
        ...
        var params = {};
        params.purcId = purcReqId;

        var opt = {
            url     : getContextPath() + "/purc/getPurcReq.do",
            async   : false,
            data    : params,
            successFn : function(data){
...
purcReq.setPurcReq = function(purcReqInfo){
    console.log(purcReqInfo)
    //consDocSeq = purcReqInfo.consDocSeq;
    $('#purcReqId').val(purcReqInfo.PURC_REQ_ID);
    $('#purcId').val(purcReqInfo.PURC_REQ_ID);
    $('#purcGroupId').val(purcReqInfo.PURC_GROUP_ID);
    $('#purcReqTitle').val(purcReqInfo.PURC_REQ_TITLE);
    $('#purcBsTitle').val(purcReqInfo.PURC_BS_TITLE);
    $('#purcBsContent').val(purcReqInfo.PURC_BS_CONTENT);
    $("#purcReqDate").data("kendoDatePicker").value(ncCom_Date(purcReqInfo.PURC_REQ_DATE, '-'));
    $('#term').val(purcReqInfo.TERM);
    $('#purcPurpose').val(purcReqInfo.PURC_PURPOSE);
    $('#txt_TR_NM').val(purcReqInfo.TR_NM).attr('code', purcReqInfo.TR_CD);
    if(purcReqInfo.CONT_TYPE_CODE_ID == "001"){
        $("#contType1").prop("checked", true);
    }else if(purcReqInfo.CONT_TYPE_CODE_ID == "002"){
        $("#contType2").prop("checked", true);
    }
    /*소액구매 파라미터 추가*/
    if(purcReqInfo.PAY_CON_CODE_ID){
        if($("#payCon").data("kendoComboBox")){
            $("#payCon").data("kendoComboBox").value(purcReqInfo.PAY_CON_CODE_ID);
        }else{
            $("#payCon").val(purcReqInfo.PAY_CON).attr('code', purcReqInfo.PAY_CON_CODE_ID);
        }
    }
    if(purcReqInfo.PAY_TYPE_CODE_ID){
        if($("#payType").data("kendoComboBox")){
            $("#payType").data("kendoComboBox").value(purcReqInfo.PAY_TYPE_CODE_ID);
        }else{
            $("#payType").val(purcReqInfo.PAY_TYPE).attr('code', purcReqInfo.PAY_TYPE_CODE_ID);
        }
    }
    $('#payCnt').val(purcReqInfo.PAY_CNT);
    if(purcReqInfo.CONT_AM){
        $('#contAm').val(purcReqInfo.CONT_AM.toString().toMoney());
    }
    $("#contDate").val(ncCom_Date(purcReqInfo.CONT_DATE, '-'));
    // $('#txt_REG_NB').val(purcReqInfo.regNb);
    // $('#C').val(purcReqInfo.ceoNm);
    if(purcReqInfo.PURC_BASIC_AM){
        $('#basicAm').val(purcReqInfo.PURC_BASIC_AM.toString().toMoney());
    }
    $('#rate').val(purcReqInfo.PURC_RATE);
    /*소액구매 파라미터 추가*/
    if(purcReqType == "3" || purcReqType == "4"){
        $('#txt_ITEM_NM').val(purcReqInfo.PURC_REQ_TITLE);
        $('#contents').val(purcReqInfo.PURC_PURPOSE);
    }
    $('#trPlace').data("kendoComboBox").value(purcReqInfo.PURC_TR_PLACE)
};
...
purcReq.setPurcReqAttachFileList = function(purcReqAttachFileList){
    $.each(purcReqAttachFileList, function(){
        ...
        $('.file_name', span).html(this.file_org_name + '.' + this.file_ext);
        $('.attachFileId', span).val(this.file_no);
        $('.fileSeq', span).val(this.file_no);
...
purcReq.purcReqInfoSave = function(){
    ...
    /** insert,Update Table Name*/
    saveObj.targetTableName = "V_PURC_INFO_TEMP";
    saveObj.targetFunction = "purcReqInfoSave";
    ...
    var opt = {
        ...
        successFn : function(data){
            if(data){
                purcReqId = data.purcReqId?data.purcReqId : "0";
                purcReqHId = data.purcReqHId?data.purcReqHId : "0";
                purcGroupId = data.purcGroupId?data.purcGroupId : "0";
                $('#purcReqId').val(purcReqId);
                $('#purcId').val(purcReqId);
                $('#purcGroupId').val(purcGroupId);
                consDocSeq = data.consDocSeq?data.consDocSeq : consDocSeq;
            }else{
                alert("오류가 발생하였습니다. 관리자에게 문의하세요");
            }
 ...
 * 첨부파일 선택창 오픈
 * */
function fnFileOpen(fileType, fileGubun){
...
function fnTpfAttachFileUpload(obj){
    ...
    $.each(fileList, function(){
        ...
        $('.attachFileId', span).val(this.file_no/*attach_file_id*/);

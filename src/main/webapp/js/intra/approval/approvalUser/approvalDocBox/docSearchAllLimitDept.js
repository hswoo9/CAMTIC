...
var docSearchAllLimitDept = {
    ...
    fnDefaultScript : function(){
        ...
        $("#startDay").kendoDatePicker({
            culture : "ko-KR",
            format : "yyyy-MM-dd",
            value : new Date(docSearchAllLimitDept.global.now.setMonth(docSearchAllLimitDept.global.now.getMonth() - 1))
        });

        $("#startDay, #endDay").attr("readonly", true);

        docSearchAllLimitDept.global.dropDownDataSource = [
            { text : "생산문서", value : "000" },
            { text : "접수문서", value : "001" },
        ];

        $("#searchCategory").kendoDropDownList({
            dataSource : docSearchAllLimitDept.global.dropDownDataSource,
            dataTextField: "text",
        ...
        docSearchAllLimitDept.gridReload();
    },

    gridReload : function() {
        docSearchAllLimitDept.global.searchAjaxData = {
            empSeq : $("#empSeq").val(),
         ...
         * 카테고리 검색조건 추가해야함
         */
        docSearchAllLimitDept.mainGrid("/approvalUser/getApprovalDocSearchListLimitDept.do", docSearchAllLimitDept.global.searchAjaxData);
    },
    ...
    deptSearchPopup : function (){
        windowPopUrl = "/common/deptBizPopup.do?";
        popName = "조직도";

...
var readerPop2 = {
    global : {
        searchAjaxData : "",
        flag : true,
        readerArr : opener.docView.global.readersArr != null ? opener.docView.global.readersArr : new Array()
    },
    ...
    fnDefaultScript : function(){
        ...
        $("#treeViewDiv").kendoTreeView({
            ...
            select: readerPop2.treeClick,
        });

        readerPop2.treeViewReload(deptSeq);
        ...
        readerPop2.readerArrCheck();
    },

    treeViewReload : function(dept){
        readerPop2.global.searchAjaxData = {
            deptSeq : dept
        }
        ...
        var deptUserDataSource = new kendo.data.DataSource({
            ...
            schema : {
                data: function (data) {
                    data.unshift(readerPop2.getDept(deptSeq));
            ...
            columns: [
                {
                    ...
                    template: function(e){
                        if(e.EMP_NAME_KR){
                            return "<button type=\"button\" class=\"k-button k-button-md k-button-solid k-button-solid-base\" onclick=\"readerPop2.addTable(" + e.EMP_SEQ + ", this, 'userClick')\">" +
                                '	<span class="k-button-text">추가</span>' +
                                '</button>';
                        }else{
                            return "<button type=\"button\" class=\"k-button k-button-md k-button-solid k-button-solid-base\" onclick=\"readerPop2.addTable(" + e.DEPT_SEQ + ", this, 'deptClick')\">" +
                                '	<span class="k-button-text">추가</span>' +
                                '</button>';
                        }
                    },
    ...
    readerArrCheck : function(){
        for(var i = 0; i < readerPop2.global.readerArr.length; i++){
            $("#readerListDataDiv").data("kendoGrid").dataSource.add({
                readerId : readerPop2.global.readerArr[i].readerId,
                SEQ : readerPop2.global.readerArr[i].readerEmpSeq,
                seqType: readerPop2.global.readerArr[i].seqType,
                readerEmpSeq: readerPop2.global.readerArr[i].readerEmpSeq,
                readerEmpName: readerPop2.global.readerArr[i].readerEmpName,
                readerDeptSeq: readerPop2.global.readerArr[i].readerDeptSeq,
                readerDeptName: readerPop2.global.readerArr[i].readerDeptName,
                readerDutyCode: readerPop2.global.readerArr[i].readerDutyCode,
                readerDutyName: readerPop2.global.readerArr[i].readerDutyName,
                readerPositionCode: readerPop2.global.readerArr[i].readerPositionCode,
                readerPositionName: readerPop2.global.readerArr[i].readerPositionName,
            });
        }
    },
    ...
    addTable : function(e, v, type){
        ...
        if(type == "userClick"){
            ...
            var result = customKendo.fn_customAjax("/user/getUserInfo", readerPop2.global.searchAjaxData);
            if(result.flag){
                readerPop2.global.flag = true;
                ...
                if($("#empSeq").val() != result.EMP_SEQ){
                    $.each($("input[name='readerPk']"), function(i, e){
                        ...
                        if(dataItem.readerEmpSeq == result.EMP_SEQ){
                            readerPop2.global.flag = false;
                        }
                    })

                    if(readerPop2.global.flag) {
                        $("#readerListDataDiv").data("kendoGrid").dataSource.add({
                            SEQ : result.EMP_SEQ,
                            seqType: "u",
            ...
            if(confirm("선택한 열람자를 삭제하시겠습니까?")){
                $.each(readerArr, function(e, i){
                    if(i.readerId != null){
                        ...
                        if(result.flag){
                            if(opener.docView.global.readersArr != null){
                                opener.docView.global.readersArr = opener.docView.global.readersArr.filter(element => element.readerId != data.readerId);
                            }
                            grid.removeRow(i.el);
                        }
                    }else{
    ...
    addReader : function(){
        ...
        $.each($("input[name='readerPk']"), function(){
            ...
            readerPop2.global.readerArr.push(data);
        })

        opener.docView.readerSelectPopClose(readerPop2.global.readerArr, readerEmpNameStr.substring(1));
        window.close();
    }
}

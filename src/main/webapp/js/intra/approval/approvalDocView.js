...
var docView = {
            ...
            columns: [
                {
                    ...
                    template : function(e){
                        ...
                        return '<button type="button" class="k-button k-rounded k-button-solid k-button-solid-base" onClick="fileViewer(\'' + e.FILE_DOWN_PATH + '\', \'' + e.fileUUID + '\')">' +
                                    '<span class="k-button-text">보기</span>' +
                                '</button>'
                    },
                    width : 60
        ...
        if($("#type").val() != "drafting" && docView.global.rs.docInfo.FORM_ID == "147"){
            ...
            if(pjtMap != null){
                ...
                if(docView.global.rs.approveNowRoute.APPROVE_EMP_SEQ == pjtMap.PM_EMP_SEQ){
                    ...
                    const fieldName = signField;
                    const empSeq = pjtMap.PM_EMP_SEQ;
    ...
    contentMod : function(){
        ...
        if(menuCd == "bustrip"){    // 출장 신청
            ...
            let url;
            if(result != null && result.PURC_SN != null){
                url = "/purc/pop/reqClaiming.do?claimSn=" + pk + "&purcSn=" + result.PURC_SN;
            }else{
                url = "/purc/pop/reqClaiming.do?claimSn=" + pk;
            }
            let name = "_self";
            let option = "width=1200, height=700, scrollbars=no, top=100, left=200, resizable=no, toolbars=no, menubar=no";
            window.open(url, name, option);

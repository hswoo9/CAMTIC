...
/** 사용자 휴가 조회 */
function getUerVacation(mode, code){
    userVacation = 0;
    $.ajax({
        url : getContextPath()+"/subHoliday/getUserVacSpecialLeaveCheck.do",
        data : "empSeq="+$("#empSeq").val(),
        dataType : "json",
        type : "post",
        async : false,
        success : function (result){
            var result = result.userVacation;
            if(mode != "onchange"){
                var html = "";
                for(var i = 0; i < result.length; i++){
                    html += "" +
                        "<tr>" +
                        "   <td>" +
                        "       <input type='hidden' id='reMainDay"+result[i].VAC_CODE_ID+"' name='"+result[i].VAC_CODE_ID+"' value='"+result[i].REMAIN_DAY+"'/>" +
                        "       "+result[i].VAC_NM+"" +
                        "   </td>" +
                        "   <td>"+result[i].APPLY_YEAR+"</td>" +
                        "   <td>"+vacMinuteToHour(result[i].GRANT_DAY)+"</td>" +
                        "   <td>"+vacMinuteToHour(result[i].MDTN_DAY)+"</td>" +
                        "   <td>"+vacMinuteToHour(result[i].USE_DAY)+"</td>" +
                        "   <td>"+vacMinuteToHour(result[i].REMAIN_DAY)+"</td>" +
                        "   <td>"+result[i].RMK+"</td>" +
                        "</tr>";
                }
                $("#userVacStat tbody").append(html);
            }else if(code != null && code != ""){
                if($.inArray(code, ['3', '4', '5']) > -1){
                    code = 1;
                }else if($.inArray(code, ['11', '12', '13']) > -1){
                    code = 10;
                }

                if($.isNumeric($("#reMainDay"+code).val())){
                    $.each($("input[name="+code+"]"), function(i, v){
                        userVacation += Number($(v).val());
                    });
                    userVacation = timeReturnToFixed(userVacation/60, 2);
                }
            }
        }
    });
...
function searchCode(){
    $.ajax({
        url : getContextPath()+"/subHoliday/getEmpVacSpecialLeaveCodeList.do",
        data : {
            mcCode : mcCode,
            mdCode : mdCode,
            empSeq : $("#empSeq").val()
    ...
    columns: [
        {
            ...
            template : function(e){
                if(e.STATUS != "111" && e.STATUS != "0" && e.STATUS != "40"){
                    if (e.AFTFAT_MNT_YN == "Y" && e.PROOF_FILE_YN == "N") {
                    ...
                    } else if (e.AFTFAT_MNT_YN == "Y" && e.PROOF_FILE_YN == "Y") {
...
function vacEditorDropDownSetting(container) {
    $.ajax({
        ...
        dataType : "json",
        success : function(result){
            var itemType2 = result.empVacCodeList;
            var defaultType = {
                "VAC_CODE_ID" : "",
                "VAC_DT_CODE_NM" : "선택"
            }
...
function vacationCal(){
    ...
    getUerVacation("onchange", $("#vacationDivision").val());

    var spclVacSetUpCode = $("#vacationDivision").data("kendoDropDownList");
    var spclVacSetUpData = spclVacSetUpCode.dataSource.view()[spclVacSetUpCode.selectedIndex];

    if(userVacation < applyHour && $("#vacationDivision").val() != ""){
        if(spclVacSetUpData.AFTFAT_MNT_YN == "N"){
            alert("잔여일수가 부족합니다.");

            if(userVacation == 0){
                $("#vacationDivision").data("kendoDropDownList").value("");
                $("#vacationStartDt").data("kendoDateTimePicker").value(new Date(nowYear, nowMonth, nowDay, '09', '00'));
                $("#vacationEndDt").data("kendoDateTimePicker").value(new Date(nowYear, nowMonth, nowDay, '09', '00'));
                $("#applyHour").val(0);
                $("#applyPeriod").val(0);
                $("#applyPeriodHourStr").text("0(0)");
            }else{
                $("#vacationStartDt").data("kendoDateTimePicker").value(new Date(nowYear, nowMonth, nowDay, '09', '00'));
                $("#vacationEndDt").data("kendoDateTimePicker").value(new Date(nowYear, nowMonth, nowDay, '18', '00'));
                $("#applyHour").val(timeReturnToFixed(8, 2));
                $("#applyPeriod").val(timeReturnToFixed(1, 3));
                $("#applyPeriodHourStr").text(timeReturnToFixed(1, 3) + "(" + timeReturnToFixed(8, 2) + ")");
            }
...
function vacProofFileViewModalOpen(e){
    viewTargetId = e;
    $('#vacProofFileViewModal').data('kendoWindow').open();
}
/** save util end */
...
function getUerVacation(mode, code){
    userVacation = 0;
    $.ajax({
        url : getContextPath()+"/subHoliday/getUserVacCheck.do",
        data : "empSeq="+$("#empSeq").val(),
        dataType : "json",
        type : "post",
        async : false,
        success : function (result){
            var result = result.userVcation;
            if(mode != "onchange"){
                var html = "";
                ...
                if($.isNumeric($("#reMainDay"+code).val())){
                    ...
                    userVacation = vacMinuteToHour(userVacation).split("/")[0];
                }
            }
        }
    });
...
function searchCode(){
    $.ajax({
        url : getContextPath()+"/subHoliday/getVacCommonCodeList.do",
        data : {
            mcCode : "V",
            empSeq : $("#empSeq").val(),
        },
...
function vacationApprovalRetrieve(docId, vacUseHistId, type){
...
function vacEditorDropDownSetting(container) {
    $.ajax({
        ...
        success : function(result){
            ...
            for(var i = 0; i<itemType2.length; i++){
                if(itemType2[i].VAC_DT_CODE_NM == "연차"){
                    itemType2.splice( i+1, 0,
                        {
                            "VAC_CODE_ID" : "3",
                            "VAC_DT_CODE_NM" : "반차"
                        }, {
                ...
                }else if(itemType2[i].VAC_DT_CODE_NM == "병가"){
                    itemType2.splice( i+1, 0,
                        {
                            "VAC_CODE_ID" : "11",
                            "VAC_DT_CODE_NM" : "병가반차"
...
function vacationCal(){
    ...
    if(userVacation < (applyHour/8) && $("#vacationDivision").val() != ""){
        alert("잔여일수가 부족합니다.");

...
<script>
	...
	$("#cmGroupCodeRegistM").kendoWindow({
		...
		open : function (){
			var htmlStr = '<input type="hidden" id="cmGroupCodeIdCR" name="cmGroupCodeIdCR">' +
						...
						'			<td>' +
						'				<input type="text" id="lgCodeNm" name="lgCodeNm" style="width: 80.5%" onkeypress="if(window.event.keyCode==13){cmGroupCodeSaveCRM();}"/>' +
						'			</td>' +
						'		</tr>' +
						'	</tbody>' +
						'</table>';
	...
	function modalKendoSetCmGroupCM(){
        ...
        var rs = customKendo.fn_customAjax("/crm/groupCodeList", data);
        console.log(rs.list);

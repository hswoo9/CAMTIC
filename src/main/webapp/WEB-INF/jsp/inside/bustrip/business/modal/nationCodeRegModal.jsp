...
<script>
	...
	function setInputData(){
		$("#cmGroupCodeIdM").val(codeP.global.gridDataItem.LG_CD);
		$("#lgCode").val(codeP.global.gridDataItem.LG_CD);
		$("#lgCodeNm").val(codeP.global.gridDataItem.LG_CD_NM);
        $("#grpSn").val(codeP.global.gridDataItem.GRP_SN);
        $("#grpNm").val(codeP.global.gridDataItem.GRP_NM);
	}
	...
	function cmCodeSaveCRM(){
		...
		if(confirm("코드를 저장하시겠습니까?")){
			if(flag){
				...
				$.ajax({
					url : "/project/insPjtCode",
					data : data,
                    type : "POST",
					dataType : "json",
					success : function(rs){
						if(rs.code == "200"){
                            codeP.cmDetailCodeList(codeP.global.gridDataItem.GRP_SN, codeP.global.gridDataItem.LG_CD);

...
<script>
	...
	function setInputData(){
		$("#cmGroupCodeIdM").val(codeB.global.gridDataItem.LG_CD);
		$("#lgCode").val(codeB.global.gridDataItem.LG_CD);
		$("#lgCodeNm").val(codeB.global.gridDataItem.LG_CD_NM);
        $("#grpSn").val(codeB.global.gridDataItem.GRP_SN);
        $("#grpNm").val(codeB.global.gridDataItem.GRP_NM);
	}
	...
	function cmCodeSaveCRM(){
		...
		if(confirm("코드를 저장하시겠습니까?")){
			if(flag){
				...
				$.ajax({
					url : "/bustrip/insNationCode",
					data : data,
                    type : "POST",
					dataType : "json",
					success : function(rs){
						if(rs.code == "200"){
                            codeB.cmDetailCodeList(codeB.global.gridDataItem.GRP_SN, codeB.global.gridDataItem.LG_CD);

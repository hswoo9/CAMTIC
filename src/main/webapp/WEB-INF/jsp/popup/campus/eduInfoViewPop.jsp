      ...
      <div class="btn-st" style="margin-top:10px; text-align:center;">
        <c:choose>
          <c:when test="${data.STATUS == 0}">
            <input type="button" class="k-button k-button-solid-info" value="결재요청" onclick="eduInfoViewPop.campusDrafting();"/>
          </c:when>
          ...
          <c:when test="${data.STATUS == 100}">
            ...
            <c:choose>
              <c:when test="${data.RES_STATUS == 1}">
                <input type="button" class="k-button k-button-solid-info" value="결과보고서 작성" onclick="eduInfoViewPop.eduResultReqPop();"/>
              </c:when>
              <c:otherwise>
